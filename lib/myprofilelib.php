...
function core_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    if ($iscurrentuser
        ...
        $node = new core_user\output\myprofile\node('contact', 'email', get_string('emailinstitucional'), null, null,
            obfuscate_mailto($user->email, ''));
        $tree->add_node($node);
    }

    //====================NOVOS USER PROFILE QUE FOROM ADICIONADO NO EVA============================

    if (!isset($hiddenfields['pemail']) && $user->pemail) {
        $node = new core_user\output\myprofile\node('contact', 'pemail', get_string('emailpersona'), null, null,
    ...
    if (!isset($hiddenfields['cargo']) && $user->cargo != "") {
        $cargo = $DB->get_record('user_cargo', array('id'=>$user->cargo));
        $node = new core_user\output\myprofile\node('contact', 'cargo', get_string('responsibility'), null, null, $cargo->name);
        $tree->add_node($node);
    }
