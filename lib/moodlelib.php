...
function user_not_fully_set_up($user, $strict = true) {
    ...
    $user->cargo = $dataUser->cargo == 0?1:$dataUser->cargo;
    //==========================================================================
//    var_dump($dataUser);exit();

    if (empty($user->firstname) or empty($user->lastname) or empty($user->email) or empty($user->cpf) or empty($user->cargo) or over_bounce_threshold($user)) {
        return true;
    }
