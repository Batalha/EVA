...
class block_eva_course_enrl_c extends block_base
{
    // Declare first
    public function init()
    {
        $this->title = get_string('eva_course_enrl_c', 'block_eva_course_enrl_c');
    }
    ...
    public function get_content()
    {
        ...
        if (is_enrolled($context, $USER, '', true)) {
          $this->content->text .='
                                  <div class="price"> '. get_string('course_enrolled', 'theme_evagu') .' </div>
                                  <p class="mt10">'. get_string('course_enrolled_text', 'theme_evagu') .'</p>';
        } else {
