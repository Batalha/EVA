...
class block_eva_course_details extends block_base
{
    // Declare first
    public function init()
    {
        $this->title = get_string('eva_course_details', 'block_eva_course_details');
    }
    ...
    public function get_content()
    {
        ...
        $ccnStringEnroled = get_string('course_enrolled', 'theme_evagu');
        $ccnStringEnroledText = get_string('course_enrolled_text', 'theme_evagu');
        ...
        if(
          ...
          && ($ccnCurrentUserId == $ccnCourse->teachers[$ccnCurrentUserId]->userId)
            ...
            $ccnStringEnroled = get_string('course_enrolled_teacher', 'theme_evagu');
            $ccnStringEnroledText = get_string('course_enrolled_teacher_text', 'theme_evagu');
        }
        ...
        if (is_enrolled($context, $USER, '', true)) {
          $this->content->text .='
            ...
            $this->content->text .='
            <div class="price">
              <i class="fa fa-credit-card"></i><span>'. get_string('course_price', 'theme_evagu') .'</span> ' . $ccnCourse->price .' <small>'. array_values($ccnCourse->priceMethods)[0]->currency .'</small>
            </div>';
           $this->content->text .='
          <a href="'. $ccnCourse->enrolmentLink .'" class="cart_btnss">'. get_string('course_enrolment', 'theme_evagu') .'</a>
          <div class="ccn-buy-access">'. get_string('course_buy_access', 'theme_evagu') .'</div>';
          if ($data->items > 0) {
          ...
          $this->content->text .= '   <div class="price">
            <i class="fa fa-credit-card"></i><span>'. get_string('course_free_accent', 'theme_evagu') .'</span> '. get_string('course_free', 'theme_evagu') .' <small>'. get_string('course_free_to_join', 'theme_evagu') .'</small>
          </div>
          <a href="'. $ccnCourse->enrolmentLink .'" class="cart_btnss">'. get_string('course_enrolment_free', 'theme_evagu') .'</a>
          <div class="ccn-buy-access">'. get_string('course_free_access', 'theme_evagu') .'</div>';
          if ($data->items > 0) {
            $this->content->text .='<ul class="price_quere_list text-left">';
          ...
          $this->content->text .= '<div class="price">'. get_string('course_error_title', 'theme_evagu') .'</div>
                                   <p class="mt10">'. get_string('course_error_text', 'theme_evagu') .'</p>';
        }
