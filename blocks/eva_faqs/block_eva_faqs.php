...
class block_eva_faqs extends block_base {

    /**
     * Start block instance.
     */
    function init() {
        $this->title = get_string('pluginname', 'block_eva_faqs');
    }
    ...
    function instance_config_save($data, $nolongerused = false) {
        ...
        for($i = 1; $i <= $data->slidesnumber; $i++) {
            ...
            file_save_draft_area_files($data->$field, $this->context->id, 'block_eva_faqs', 'slides', $i, $filemanageroptions);
        }
    ...
    function instance_delete() {
        global $DB;
        $fs = get_file_storage();
        $fs->delete_area_files($this->context->id, 'block_eva_faqs');
        return true;
    }
    ...
    public function instance_copy($fromid) {
        ...
        for($i = 1; $i <= $data->slidesnumber; $i++) {
            ...
            if (!$fs->is_area_empty($fromcontext->id, 'block_eva_faqs', 'slides', $i, false)) {
                $draftitemid = 0;
                file_prepare_draft_area($draftitemid, $fromcontext->id, 'block_eva_faqs', 'slides', $i, $filemanageroptions);
                file_save_draft_area_files($draftitemid, $this->context->id, 'block_eva_faqs', 'slides', $i, $filemanageroptions);
            }
        }
