...
class block_eva_slider_8 extends block_base {

    /**
     * Start block instance.
     */
    function init() {
        $this->title = get_string('pluginname', 'block_eva_slider_8');
    }
    ...
    function specialization() {
        ...
        if (empty($this->config)) {
          ...
          $this->config->title_1 = 'With evagu, Learning ';
          ...
          <div class="home14-slider style2">
        		<div class="container-fluid p0">
                  ...
                  for ($i = 1; $i <= $data->slidesnumber; $i++) {
                      ...
                      $files = $fs->get_area_files($this->context->id, 'block_eva_slider_8', 'slides', $i, 'sortorder DESC, id ASC', false, 0, 0, 1);
                      if (!empty($data->$sliderimage) && count($files) >= 1) {
                        $mainfile = reset($files);
                        $mainfile = $mainfile->get_filename();
                        $data->$sliderimage = moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php", "/{$this->context->id}/block_eva_slider_8/slides/" . $i . '/' . $mainfile);
                      } else {
                ...
                <div class="row">
                  <div class="col-lg-12">
                    <div class="bg-wave"><img src="'.$CFG->wwwroot.'/theme/evagu/images/background/wave.svg" alt=""></div>
                  </div>
                </div>
              </div>
            </div>
    ...
    function instance_config_save($data, $nolongerused = false) {
        ...
        for($i = 1; $i <= $data->slidesnumber; $i++) {
            ...
            file_save_draft_area_files($data->$field, $this->context->id, 'block_eva_slider_8', 'slides', $i, $filemanageroptions);
        }
    ...
    function instance_delete() {
        global $DB;
        $fs = get_file_storage();
        $fs->delete_area_files($this->context->id, 'block_eva_slider_8');
        return true;
    }
    ...
    public function instance_copy($fromid) {
        ...
        for($i = 1; $i <= $data->slidesnumber; $i++) {
            ...
            if (!$fs->is_area_empty($fromcontext->id, 'block_eva_slider_8', 'slides', $i, false)) {
                $draftitemid = 0;
                file_prepare_draft_area($draftitemid, $fromcontext->id, 'block_eva_slider_8', 'slides', $i, $filemanageroptions);
                file_save_draft_area_files($draftitemid, $this->context->id, 'block_eva_slider_8', 'slides', $i, $filemanageroptions);
            }
        }
