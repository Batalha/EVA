...
require_once($CFG->dirroot. '/theme/evagu/ccn/general_handler/ccn_video_handler.php');
require_once($CFG->dirroot. '/theme/evagu/ccn/block_handler/ccn_block_handler.php');
class block_eva_course_intro extends block_base {
    public function init() { $this->title = get_string('eva_course_intro', 'block_eva_course_intro'); }
    ...
    public function get_content() {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->context->id, 'block_eva_course_intro', 'content');
        ...
        $eva_share_fb = 'https://www.facebook.com/sharer/sharer.php?u='. $this->page->url;
        $white = '';
              ...
              <div class="ccn-identify-course-intro">
                <div class="cs_instructor">
                  ...
                  if($PAGE->theme->settings->coursecat_modified != 1){
                    $this->content->text .='  <li class="list-inline-item"><a class="'.$white.'">'.get_string('last_updated', 'theme_evagu').' '. userdate($COURSE->timemodified, get_string('strftimedate', 'langconfig'), 0) .'</a></li>';
                  }
                  $this->content->text .='
                  </ul>
                  <ul class="cs_watch_list float-right mb0">
                    <li class="list-inline-item"><a class="'.$white.'" target="_blank" href="'.$eva_share_fb.'"><span class="flaticon-share"> '.get_string('share','theme_evagu').'</span></a></li>
                  </ul>
                </div>
                ...
                if($PAGE->theme->settings->coursecat_enrolments != 1){
                  $this->content->text .='<li class="list-inline-item"><a class="'.$white.'" href="#"><span class="flaticon-profile"></span> '. count_enrolled_users($context) .' '.get_string('students_enrolled', 'theme_evagu').'</a></li>';
                }
                if($PAGE->theme->settings->coursecat_announcements != 1){
                  $this->content->text .='<li class="list-inline-item"><a class="'.$white.'" href="#"><span class="flaticon-comment"></span> '. $ccnCourse->numberOfSections .' '.get_string('topics', 'theme_evagu').'</a></li>';
                }
