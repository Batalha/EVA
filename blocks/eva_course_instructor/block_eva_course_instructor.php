...
class block_eva_course_instructor extends block_base
{
    public function init()
    {
        $this->title = get_string('eva_course_instructor', 'block_eva_course_instructor');
    }
    ...
    public function specialization() {
      ...
      if (empty($this->config)) {
        $this->config->title = 'Sobre o instrutor(a)';
    ...
    public function get_content()
    {
        ...
        if(!empty($this->config->position)){
          ...
          $renderPosition = get_string('config_teacher', 'theme_evagu');
        }
        if(!empty($this->config->students)){
          ...
          if($ccnUser->teachingStudentCount !== 0) {
            ...
            $renderStudents = get_string('no_students_yet', 'theme_evagu');
          }
        } else {
        ...
        if(!empty($this->config->courses)){
          ...
          if($ccnUser->teachingCoursesCount !== 0) {
            ...
            $renderCourses = get_string('no_courses_yet', 'theme_evagu');
          }
        } else {
          $renderStudents = '';
        }
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->context->id, 'block_eva_course_instructor', 'content');
        $ccnImage = '';
        ...
        if(!empty($ccnImage)){
          ...
          $renderImage = '<img src="' . $CFG->wwwroot .'/theme/evagu/images/team/6.png" alt="" />';
        }
