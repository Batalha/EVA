...
class block_eva_course_rating extends block_base {
    public function init() {
        $this->title = get_string('pluginname', 'block_eva_course_rating');
    }
    ...
    function specialization() {
        ...
        if (empty($this->config)) {
          $this->config->title = 'Feedback dos Alunos';
        }
    }
    ...
    public function get_content() {
        ...
        if(!empty($this->config->title)){$this->content->title =  format_text($this->config->title, FORMAT_HTML, array('filter' => true));} else {$this->content->title = get_string('pluginname', 'block_eva_course_rating');}

        $courseid = $COURSE->id;
        $context = get_context_instance(CONTEXT_COURSE, $courseid);

        $canRate = has_capability('block/eva_course_rating:rate', $context);
        if($canRate == 1) {
    ...
    public function count_ratings($courseID) {
        global $CFG, $DB;
        $countRecords = $DB->count_records('theme_evagu_courserate', array('course'=>$courseID));
        $countRatings = '';
        if ($countRecords > 0) {
            $countRatings = get_string ('rated_by', 'theme_evagu', $countRecords);
        } else {
            $countRatings = get_string ('rated_by_none', 'theme_evagu');
        }
        return $countRatings;
    }
    ...
    public function get_specific_average($courseID, $rating) {
        global $CFG, $DB;
        $countOnlyRating        = $DB->count_records('theme_evagu_courserate', array('course'=>$courseID, 'rating'=>$rating));
        ...
        $countTotal             = $DB->count_records('theme_evagu_courserate', array('course'=>$courseID));
    ...
    public function submit_rating() {
      ...
      $return =   '<form id="ccn-star-rate" method="post" action="'.$CFG->wwwroot.'/theme/evagu/ccn/form_handler/ccn_rate_course.php">
      ...
      $return .= '  </div>
                    <button class="btn btn-primary" type="submit">'.get_string('rate_course', 'theme_evagu').'</button>
                  </form>';
      return $return;
    }
