...
class block_eva_course_feat_a_edit_form extends block_edit_form {
    protected function specific_definition($mform) {
        ...
        for($i = 1; $i <= $data->slidesnumber; $i++) {
            $mform->addElement('header', 'config_header' . $i , get_string('config_item', 'theme_evagu') . $i);
            ...
            $mform->addElement('text', 'config_item_subtitle' . $i, get_string('config_subtitle', 'theme_evagu', $i));
            $mform->setDefault('config_item_subtitle' .$i , '6');
            $mform->setType('config_item_subtitle' . $i, PARAM_TEXT);

            // $mform->addElement('text', 'config_item_icon' . $i, get_string('config_icon_class', 'theme_evagu', $i));
    ...
    function set_data($defaults) {
        if (!empty($this->block->config) && is_object($this->block->config)) {
            ...
            for($i = 1; $i <= $this->block->config->slidesnumber; $i++) {
                ...
                file_prepare_draft_area($draftitemid, $this->block->context->id, 'block_eva_course_feat_a', 'slides', $i, array('subdirs'=>false));
