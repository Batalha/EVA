...
class block_eva_about_1_edit_form extends block_edit_form
{
    ...
    function set_data($defaults)
    {
        ...
        file_prepare_draft_area($draftitemid, $this->block->context->id, 'block_eva_about_1', 'content', 0,
            array('subdirs' => true));
        $entry->attachments = $draftitemid;
        parent::set_data($defaults);
        if ($data = parent::get_data()) {
            file_save_draft_area_files($data->config_image, $this->block->context->id, 'block_eva_about_1', 'content', 0,
                array('subdirs' => true));
        }
        ...
        if (!empty($this->block->config) && is_object($this->block->config)) {
            ...
            $defaults->config_body['text'] = file_prepare_draft_area($draftid_editor, $this->block->context->id, 'block_eva_about_1', 'content', 0, array('subdirs'=>true), $currenttext);
