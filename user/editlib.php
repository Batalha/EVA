...
function useredit_shared_definition(&$mform, $editoroptions, $filemanageroptions, $user) {
    ...
    for ($i = 0; $i <= count($usercargo); $i++){
        $cargo[$i] = $usercargo[$i-1];
    }
    $default_cargo[''] = get_string('selectacargo');
    $cargo = array_merge($default_cargo, $usercargo);
    $mform->addElement('select', 'cargo', get_string('responsibility'), $cargo);
    $mform->addRule('cargo', $strrequired, 'required', null, 'client');
    ...
    //================= MATRICULA SIAPE ADICIONADO ========================
    $mform->addElement('text', 'siape', get_string('siape'), 'maxlength="30" size="25"');
    $mform->setType('siape', core_user::get_property_type('siape'));
    $mform->setForceLtr('siape');
