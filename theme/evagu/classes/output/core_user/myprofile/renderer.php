  ...
  public function render_tree(tree $tree) {
      ...
      if(!($ccn_page->id == $USER->id)) {
        ...
        if ($SESSION->theme_evagu_counter[$ccn_page->id]['time'] < (time() - $ccn_time_difference)) {
          ...
          $DB->insert_record('theme_evagu_counter', $dataobject, false);
      ...
      <section class="our-team">
		    <div class="">';
          if($userAvatar && $PAGE->theme->settings->user_profile_layout != 1){ // evaguFront ONLY
            $return .='
            ...
            <div class="'.$ccn_col_main.'">
              <div class="row">
                ...
                <div class="'.$ccn_col_main_block.'">
                  <div class="row">
                    <div class="col-lg-12">
                      <h3 class="r_course_title">'.$userFirst.get_string('apostrophe_s', 'theme_evagu').' '.get_string('last_accessed_courses', 'theme_evagu').'</h3>
                    </div>';
                    $ia = 0;
                        ...
                        $return .='
                          <div class="thumb">
                            ...
                            <div class="overlay">
                              ...
                              <a class="tc_preview_course" href="'.$courseLink.'">'.get_string('preview_course', 'theme_evagu').'</a>
                            </div>
                          </div>';
                        }
                        $return .='
              ...
              } elseif ($userLastCourses && $PAGE->theme->settings->user_profile_layout == 1){ //evagu Dash
                $return .='
                <div class="'.$ccn_col_main_block.' p0">
                  <div class="'.$ccn_col_block_title.'">
                    <h4>'.$userFirst.get_string('apostrophe_s', 'theme_evagu').' '.get_string('last_accessed_courses', 'theme_evagu').'</h4>
                  </div>
                            ...
                            <div class="overlay">
                              <ul class="mb0">
                                <li class="list-inline-item">
                                  <a class="mcc_view" href="'.$courseLink.'">'.get_string('view', 'theme_evagu').'</a>
                                </li>
                              </ul>
                            </div>
                          </div>';
                            ...
                            <div class="mc_content">
                              <div class="ccn_mc_content_header">
                                <div class="ccn_mc_content_header_details">
                                  ...
                                  <div class="ccn_mc_content_header_status"><small class="tag">'.get_string('published', 'theme_evagu').'</small></div>
                                </div>
                              </div>
                              '.$courseDesc.'
                            </div>
              ...
              if($userSkype || $userIcq || $userYahoo | $userAim || $userMsn){
                $return .='
  							<p>'.get_string('socialmedia', 'theme_evagu').'</p>
              ...
              ';
                if($teachingStudentCount){
                  $return .='
    							<p>'.get_string('studentsiamteaching', 'theme_evagu').'</p>
    							<i>'.$teachingStudentCount.'</i>';
                }
                if($teachingCoursesCount){
                  $return .='
    							<p>'.get_string('coursesiamteaching', 'theme_evagu').'</p>
    							<i>'.$teachingCoursesCount.'</i>';
                }
                  $return .='<p>'.get_string('coursesiamtaking', 'theme_evagu').'</p>
    							<i>'.$enrolmentCount.'</i>';
                if($ccnProfileCount){
                  $return .='<p>'.get_string('profileviews', 'theme_evagu').'</p>
    							<i>'.$ccnProfileCount.'</i>';
                }
                $return .='
						</div>
