...
//  * @return string
//  */
// function theme_evagu_get_main_scss_content($theme) {
//     global $CFG;
//
//     $scss = '';
...
//     //
//     // } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_evagu', 'preset', 0, '/', $filename))) {
//     //     // This preset file was fetched from the file area for theme_evagu and not theme_boost (see the line above).
...
//
//
//     // $scss .= file_get_contents($CFG->dirroot . '/theme/evagu/scss/evagu-boost-preprocess.scss');
//     $scss .= file_get_contents($CFG->dirroot . '/theme/boost/scss/moodle.scss');
//     // $scss .= file_get_contents($CFG->dirroot . '/theme/evagu/scss/evagu-boost-postprocess.scss');
//
//     // // Pre CSS - this is loaded AFTER any prescss from the setting but before the main scss.
//     // $pre = file_get_contents($CFG->dirroot . '/theme/evagu/scss/pre.scss');
//     // // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
//     // $post = file_get_contents($CFG->dirroot . '/theme/evagu/scss/post.scss');
//
...
function theme_evagu_update_settings_images($settingname) {
    ...
    // The setting name that was updated comes as a string like 's_theme_evagu_loginbackgroundimage'.
...
function theme_evagu_process_css($css, $theme) {
    ...
    if(is_null($replacement)){$replacement = $CFG->wwwroot . '/theme/evagu/images/background/inner-pagebg.jpg';}
    $css = str_replace($tag, $replacement, $css);
    ...
    if(is_null($replacement)){$replacement = $CFG->wwwroot . '/theme/evagu/images/preloader.gif';}
    $css = str_replace($tag, $replacement, $css);
